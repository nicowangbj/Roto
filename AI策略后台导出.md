...
- 类型：`conversation`
- 状态：已配置
- 触发时机：用户选择「没有课题」后进入导师对话
- 描述：AI导师通过多轮对话收集用户兴趣、能力、时间等信息

### Prompt Template

```text
你是一位温和、耐心的探究导师，正在和一位学生进行选题前的沟通。
...
- 状态：已配置
- 触发时机：信息收集对话结束后
- 描述：基于对话内容生成结构化的用户初步报告

### Prompt Template

```text
基于以下对话记录，生成一份结构化的学生画像报告。
...
- 状态：已配置
- 触发时机：用户确认画像报告后
- 描述：从广泛角度生成事件/关键词/方向推荐列表

### Prompt Template

```text
基于学生画像，生成一组研究方向关键词供学生选择。
...
- 状态：已配置
- 触发时机：用户完成关键词勾选后
- 描述：基于用户选择的方向展示相关研究题目列表

### Prompt Template

```text
基于学生选择的关键词，生成相关的研究案例列表供参考。
...
- 状态：已配置
- 触发时机：两轮筛选后
- 描述：生成具体课题推荐，每个课题附带详细讲解

### Prompt Template

```text
基于学生的画像和两轮筛选结果，生成 3-5 个具体的课题推荐。
...
- 状态：已配置
- 触发时机：用户对计划提出异议时
- 描述：导师与用户对话协商，理解诉求后调整计划

### Prompt Template

```text
你是一位善于倾听的探究导师，学生对当前的研究计划有疑问或想要调整。
...
## AI-S10 地图节点状态与进度计算策略

- 模块：`plan_display`
- 类型：`computation`
- 状态：已配置
- 触发时机：用户进入科研地图时
- 描述：计算每个阶段/节点的完成状态和整体进度
...
- 状态：已配置
- 触发时机：用户在任务执行中点击导师咨询
- 描述：以提问方式引导用户思考，提供启发但不直接给答案

### Prompt Template

```text
你是一位苏格拉底式的探究导师，学生在执行任务时向你寻求帮助。
...
- 状态：已配置
- 触发时机：全局配置
- 描述：定义A/B/C/D评分的统一标准框架

### Prompt Template

```text
定义产出评分的统一标准框架。此策略为全局配置，被其他评分策略引用。
...
## AI-S18 二次提交评分策略（含上限规则）

- 模块：`plan_execution`
- 类型：`evaluation`
- 状态：已配置
- 触发时机：用户使用Hint后重新提交
- 描述：对重新提交的产出评分，执行上限规则（Hint后最高C）

### Prompt Template

```text
对使用 Hint 后重新提交的产出进行评分，并执行上限规则。
...
- 状态：已配置
- 触发时机：主动或被动触发计划调整时
- 描述：导师与用户沟通了解原因，提出调整方案建议

### Prompt Template

```text
你是一位关心学生的探究导师，发现学生的进度出现了问题，需要和学生沟通。
...
- 类型：`generation`
- 状态：已配置
- 触发时机：任务完成、阶段完成等关键事件后
- 描述：基于研究过程自动生成探究日志条目

### Prompt Template

```text
基于研究过程中的关键事件，自动生成一条探究日志。
...
- 状态：已配置
- 触发时机：日志生成时
- 描述：将研究过程数据转写为第一人称视角的日志内容

### Prompt Template

```text
将结构化的研究过程数据转写为第一人称视角的自然语言日志。
